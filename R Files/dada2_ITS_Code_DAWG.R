...
# first, we need to delete the sample that has 0 reads that passed step 4
# Which is sample 21 on row 14
filtFs.df<-as.data.frame(filtFs) #makes the character string into a df
filtFs.df<-filtFs.df[-c(14), ] #deletes row 14 (sample 21) from df
filtFs.1<-as.character(filtFs.df) #converts df into character string

# learn error rates
# error models are run specific
errf<- learnErrors(filtFs.1, multithread=TRUE) #set multithread=FALSE if on windows
...
# need to delete sample 21 from this file
sample.names.df<-as.data.frame(sample.names)
sample.names.df<-sample.names.df[-c(14), ]
sample.names.1<-as.character(sample.names.df)
...
# set multithread=FALSE if usin windows
...
dim(seqtab)
#29 samples with 665 ESVs

#### 8. Remove Chimeric Sequences ####
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE, verbose=TRUE)
#142 seqs were removed because they were chimeras
dim(seqtab.nochim)
# 29 samples with 523 ESVs
